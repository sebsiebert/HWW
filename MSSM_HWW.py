...
class MSSMHiggsModel(PhysicsModel):
    ...
    def safeTH2DivideForKappaDelta(self, h1, h2, coeff=1.):
        ...
        if h1.GetNbinsX() != h2.GetNbinsX() or h1.GetNbinsY() != h2.GetNbinsY():
            print 'Warning: Syst. histograms are not of same size!'
            print 'h1: X =',h1.GetNbinsX(),', Y =',h1.GetNbinsY()
            print 'h2: X =',h2.GetNbinsX(),', Y =',h2.GetNbinsY()
        for x in xrange(1, h1.GetNbinsX() + 1):
            for y in xrange(1, h2.GetNbinsY() +1):
                val_h1 = h1.GetBinContent(x, y)
                val_h2 = h2.GetBinContent(x, y)
                #print x,y,":",val_h1,',',val_h2
                if val_h1 == 0. or val_h2 == 0. or val_h1 != val_h1 or val_h2 != val_h2:
                    print ('Warning: dividing histograms %s and %s at bin (%i,%i)=(%g, %g) '
                           'with values: %g/%g, will set the kappa to 1.0 here' % (
                                h1.GetName(), h2.GetName(), x, y, h1.GetXaxis().GetBinCenter(x),
                                h1.GetYaxis().GetBinCenter(y), val_h1, val_h2
                    ...
                    if new_val != new_val:
                        ...
                        print new_val,'= (',val_h2,'+',coeff,'*',val_h1,') /',val_h2
                        new_val = 1.
                    #if abs(new_val-1)>0.5: print x,y,":",new_val
                res.SetBinContent(x, y, new_val)
        return res
...
#            self.modelBuilder.out.factory('prod::%s(%s,%s)' % (name.format(X=X, LC=loopcontrib), name.format(X=X, LC=""), "gg%s_%s_MSSM_frac" % (X,loopcontrib))) #multiply t,b,i fractions with xsec at NNLO
        ...
        for era, (file, version) in self.modelFiles.iteritems():
            ...
            #! [part3]
                ...
                if version == 0:
                    ...
                    # ... and in the even newer versions they
                    # are absolute differences (also for QCD scale)
                    self.doAsymPow('systeff_xs_gg%s_pdf_%s' % (X,era),
                        self.safeTH2DivideForKappaDelta(f.Get(hd['xs_gg%s_pdf_lo'%X]), f.Get(hd['xs_gg%s'%X]), +1.),
                        self.safeTH2DivideForKappaDelta(f.Get(hd['xs_gg%s_pdf_hi'%X]), f.Get(hd['xs_gg%s'%X]), +1.),
                        'xs_gg%s_pdf_%s' % (X,era), pars)
                ...
                self.PROC_SETS.append(([ 'gg%s'%X, 'qq%s'%X ], [ '%sWWSBI'%X], [era]))
                self.PROC_SETS.append(([ 'gg%s'%X, 'qq%s'%X ], [ 'SBIbkg'], [era]))
                #if self.ggHatNLO != None:
    ...
    def doParametersOfInterest(self):
        ...
        #self.modelBuilder.doVar("r_SM[1,0,20]") # TODO: What to do with this?
        #self.modelBuilder.out.var('r_SM').setConstant(True)

        self.modelBuilder.factory_("expr::S_SCALE(\"@0 - TMath::Sqrt(@0)\", r)")
        #self.modelBuilder.factory_("expr::B_SCALE(\"1.-TMath::Sqrt(@0)\",r)")
        self.modelBuilder.factory_("expr::B_SCALE(\"TMath::Sqrt(@0)\",r)") #Add "1-this" later -> See "wholestring"
        self.modelBuilder.factory_("expr::SBI_SCALE(\"TMath::Sqrt(@0)\",r)")
        ...
        for proc_set in self.PROC_SETS:
            for (P, D, E) in itertools.product(*proc_set):
                print (P, D, E)
                #if (self.SMSignal not in D): #altenative hypothesis if SMSignal not in process name
                Dtemp = "HWW" if D in ["HWWSBI", "SBIbkg"] else D
                terms = ['xs_%s_%s' % (P, E), 'br_%s_%s'% (Dtemp, E)]
                if D == "HWW": terms += ['S_SCALE']
                if D == "HWWSBI": terms += ['SBI_SCALE']
                if D == "SBIbkg": terms += ['B_SCALE']
                ...
                if D == "SBIbkg":
                    ...
                    for index,term in enumerate(terms):
                        thisstring1 += "@"+str(index)
                        thisstring2 += term
                        if index+1 < len(terms):
                            thisstring1 += "*"
                            thisstring2 += ", "
                    wholestring = "expr::scaling_"+P+"_"+D+"_"+E+"(\"1.-("+thisstring1+")\", "+thisstring2+")"
                    #print wholestring
                    self.modelBuilder.factory_(wholestring)
                else:
    ...
    def getYieldScale(self,bin,process):
        ...
        if self.DC.isSignal[process]:
            ...
            return scaling
        elif process in ["ggWW", "qqWWqq", "ggH_hww", "qqH_hww"]:# and not (("dy" in bin) or ("top" in bin)) <-- Add this when removing signal from CR
            if process in ["ggWW", "ggH_hww"]: scaling = 'scaling_%s_%s_%s' % ("ggH", "SBIbkg", "13TeV")
            if process in ["qqWWqq", "qqH_hww"]: scaling = 'scaling_%s_%s_%s' % ("qqH", "SBIbkg", "13TeV")
            print 'Scaling %s/%s as %s' % (bin, process, scaling)
            return scaling
        else:
